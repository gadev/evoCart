<?php
/*
 * на случай, если есть еще плагины на OnPageNotFound,
 * evoCart должен вызываться перед ними
 * и для этого чекаем, и если есть, то даем перейти к ним
 */
...
if($e->name == 'OnPageNotFound') {
    if (empty($_SERVER['HTTP_X_REQUESTED_WITH']) && 	 strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
    	$modx->sendRedirect($modx->makeUrl($modx->getConfig('error_page')));
    }
